...
int *GenerateRandomArray(const int max_size, const int max_value, int len) {
    ...
    for (int i = 0; i < len; ++i) {
        // [-?, ?]
        // arr[i] = static_cast<int>((max_value + 1) * (rand() / (RAND_MAX + 1.0)) - static_cast<int>(max_value *
        // (rand() / (RAND_MAX + 1.0)))); arr[i] = static_cast<int>((max_value + 1) * (rand() / (RAND_MAX + 1.0)) -
        // max_value * (rand() / RAND_MAX + 1.0));
...
bool ExistBinary(const int *arr, const int num, const int len) {
    ...
    int l = 0, r = len - 1;
    while (l < r) {
        // N * 2 + 1   ===   (N << 1) | 1  左移一位乘 2 ， 或 1， 把左移用 0 补的那一位，变 1，相当于加 1  位运算 比
        // 加减快 比 乘除快
...
int main(int argc, char *argv[]) {
    ...
    // constexpr int num = 5, len = static_cast<int>(std::size(arr));  // std::size() 是 c++17
    // 的新标准库函数，获取容器或数组的大小，如vector，array等；对于 new 的数组不能使用 const bool is_exist =
    // ExistBinary(arr, num, len); std::cout << (is_exist ? "Nice" : "Fucking Fucked!") << std::endl;
    ...
    // 	int len;
    // 	const int value = static_cast<int>((max_value + 1) * (rand() / (RAND_MAX + 1.0)) - static_cast<int>(max_value *
    // (rand() / (RAND_MAX + 1.0)))); 	int* arr = GenerateRandomArray(max_size, max_value, len); 	std::sort(arr, arr +
    // len);
    //
    // 	std::cout << "Arr: ";
    // 	for (int j = 0; j < len; j++) {
